def find_next_empty(puzzle):
    ...
    # keep in ind that we are using 0-8 for our indices
    for r in range(9):
        for c in range(9):
            if puzzle[r][c] == -1:
                return r, c
...
def is_valid(puzzle, guess, row, col):
    ...
    # if we got here, these checks pass
    return True 
...
def solve_sudoko(puzzle):
    ...
    # mutates puzzle to be the soltion (if solution exists)
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3,....., 9
        ...
        puzzle[row][col] = -1 # rreset the game
